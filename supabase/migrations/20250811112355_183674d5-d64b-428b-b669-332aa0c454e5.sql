...
create index if not exists idx_task_ai_messages_task_user on public.task_ai_messages (task_id, user_id, created_at);

-- Policy: Users can view messages for tasks they have access to
create policy if not exists "Users can view AI messages for accessible tasks"
...
create policy if not exists "Users can insert their own AI messages"
  on public.task_ai_messages
  for insert
  with check (
